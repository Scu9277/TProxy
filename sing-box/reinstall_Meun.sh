...
# ----------------------------------------------------------------
install_renetwork() {
    echo -e "${BLUE}--- 正在执行 [组件 7: 配置网卡IP] ---${NC}"
...
main_menu() {
    clear
    echo "=================================================="
    echo "     Mihomo / Sing-box 模块化安装脚本 (V6)"
    ...
    echo "  3) 仅安装 Docker"
    echo "  4) 仅安装 Sub-Store (需 Docker)"
    echo "  5) 仅安装 TProxy (tproxy_内核转发)"
    echo "  6) 仅安装 DNS 劫持 (/etc/hosts)"
    echo "  7) 仅安装 配置网卡IP (renetwork.sh)" # 【 V6 变更 】
    echo "--------------------------------------------------"
    echo "  8) 退出脚本" # 【 V6 变更 】
    echo
    read -p "请输入选项 [1-8]: " choice # 【 V6 变更 】
    ...
    case $choice in
        ...
        8) # 【 V6 变更 】
            ...
            ;;
        *)
            echo -e "${RED}无效选项，请输入 1 到 8。${NC}" # 【 V6 变更 】
            sleep 2
            ;;
    esac
    
    if [ "$choice" != "8" ]; then # 【 V6 变更 】
        # 循环显示主菜单，除非选择退出
        read -p "按任意键返回主菜单..."
        main_menu
    fi
