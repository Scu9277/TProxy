...
# 🛑🛑🛑【【【【 重要：请在这里填入你的蓝奏云分享链接 】】】】🛑🛑🛑
#
# 示例: CONFIG_ZIP_URL="https://shangkouyou.lanzouo.com/iYjkG39j4s0j"
#
# ‼️‼️‼️ 注意：V8 版要求您 ZIP 包内的 config.yaml 提前写好 DNS 劫持规则，
#           并使用 10.0.0.121 作为占位符 IP。
# ‼️‼️‼️
CONFIG_ZIP_URL="https://shangkouyou.lanzouo.com/iYjkG39j4s0j"
# 占位符 IP (与您 config.yaml 中的保持一致)
...
setup_config() {
    if [ -f "/etc/mihomo/config.yaml" ]; then
        ...
        echo -e "${YELLOW}    (如果上次启动失败，config.yaml 可能已损坏，推荐选择 'y' 覆盖)${NC}"
...
get_lan_ip() {
    ...
    # 使用 hostname -I 获取 IP，awk 提取第一个
    LAN_IP=$(hostname -I | awk '{print $1}')

    if [ -z "$LAN_IP" ]; then
        echo -e "${RED}❌ 未能自动获取局域网 IP！DNS 劫持将失败。${NC}"
        echo -e "${YELLOW}请检查 'hostname -I' 命令。${NC}"
        exit 1
    else
        echo -e "${GREEN}✅ 本机 IP: $LAN_IP (将用于 DNS 劫持配置)${NC}"
    fi
    echo "----------------------------------------------------------------"
}

# 替换 Mihomo DNS 占位符 (局域网劫持)
replace_placeholder_ip() {
    CONFIG_FILE="/etc/mihomo/config.yaml"
    echo -e "🔄 正在检查并替换 Mihomo 配置中的占位符 IP..."

    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${RED}❌ 未找到 $CONFIG_FILE！跳过 IP 替换。${NC}"
        echo "----------------------------------------------------------------"
        return
    fi
    ...
    if grep -q "$PLACEHOLDER_IP" "$CONFIG_FILE"; then
        if [ -z "$LAN_IP" ]; then
            echo -e "${RED}❌ LAN_IP 为空！无法执行替换。${NC}"
            exit 1
        fi
        ...
        # 修复：验证逻辑
        if [ "$PLACEHOLDER_IP" == "$LAN_IP" ] || ! grep -q "$PLACEHOLDER_IP" "$CONFIG_FILE"; then
            echo -e "${GREEN}✅ 占位符 IP 替换成功！${NC}"
        else
            echo -e "${RED}❌ 占位符 IP 替换失败！请检查文件权限或 sed 命令。${NC}"
            exit 1
        fi
    else
        echo -e "${GREEN}👍 未在 $CONFIG_FILE 中检测到占位符 ${PLACEHOLDER_IP}，假定已配置。${NC}"
    fi
    echo "----------------------------------------------------------------"
}
...
start_mihomo_service() {
    ...
    if systemctl is-active --quiet mihomo; then
        ...
        echo -e "${YELLOW}常见原因：config.yaml 格式错误 (如 IP 替换失败) 或端口被占用 (如 systemd-resolved)。${NC}"
        exit 1
    fi
    echo "----------------------------------------------------------------"
}
...
main() {
    ...
    # 第 3.5 步：配置本机 Hosts 文件
    echo -e "📝 ${YELLOW}第 3.5 步：配置本机 Hosts 文件...${NC}"
    setup_hosts_file
    ...
    # 第 6.5 步：配置 DNS 劫持
    echo -e "🌐 ${YELLOW}第 6.5 步：配置 DNS 劫持...${NC}"
    get_lan_ip             # 获取 $LAN_IP
    replace_placeholder_ip # 替换 config.yaml 中的占位符

    # 启动 Mihomo (在 DNS 配置之后)
    start_mihomo_service
    ...
    echo -e "主机名: ${YELLOW}scu.lan${NC} (或 21.cn / 21.wifi 等)"
    echo -e "端口: ${YELLOW}80${NC}"
    echo -e "密码: ${YELLOW}1381ni${NC} (请按实际配置为准)"
    echo ""
