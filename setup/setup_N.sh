...
# ----------------------------------------------------------------
#   组件 5: TProxy (Nftables 版本)
# ----------------------------------------------------------------
install_tproxy() {
    echo -e "${BLUE}--- 正在安装 [组件 5: TProxy (Nftables 版本)] ---${NC}"
    echo -e "🔧 准备执行 TProxy Nftables 脚本 (TProxy_Nftables.sh)..."
    
    TPROXY_SCRIPT_URL="https://ghfast.top/raw.githubusercontent.com/Scu9277/TProxy/refs/heads/main/Nftables/TProxy_Nftables.sh"
    
    if bash <(curl -sSL "$TPROXY_SCRIPT_URL"); then
        echo -e "${GREEN}✅ TProxy Nftables 脚本 (TProxy_Nftables.sh) 执行完毕！${NC}"
    else
        echo -e "${RED}❌ TProxy Nftables 脚本执行失败。${NC}"
    fi
    echo "----------------------------------------------------------------"
}
...
show_logo() {
    ...
    echo " "
    echo -e "     作者: ${GREEN}Scu x Duang${NC}"
...
main_menu() {
    ...
    echo "  6) 安装 TProxy (Nftables 版本)" 
    echo "  7) 安装 DNS 劫持 (/etc/hosts)"
    echo "  8) 配置网卡IP (renetwork.sh)"
    echo
    echo -e "--- ${RED}高级系统工具 ${NC}---"
    echo "  9) 更改主机名 (Hostname)"
    echo " 10) 运行 科技Lion 优化脚本 (kejilion.sh)"
    echo -e " 11) ${YELLOW}系统深度清理 (卸载Docker/清缓存/释内存)${NC}"
    echo -e " 12) ${RED}一键重装系统 (Debian 13 - 极度危险!)${NC}"
    echo "--------------------------------------------------"
    echo -e " ${MAGENTA}00) 退出脚本${NC}"
    echo
    read -p "请输入选项 [1-12 或 00]: " choice
    ...
    case $choice in
        ...
        8)
            ...
            exit 0
            ;;
        9) install_change_hostname ;;
        10) install_kejilion_optimizer ;;
        11) install_system_cleanup ;;
        12) install_reinstall_os ;;
        00)
            echo "退出脚本。"
            exit 0
            ;;
        *)
            echo -e "${RED}无效选项，请输入 1 到 12 或 00。${NC}"
            sleep 2
            ;;
    esac
    
    # 循环显示主菜单
    if [ "$choice" != "00" ] && [ "$choice" != "8" ] && [ "$choice" != "12" ]; then
        read -p "按任意键返回主菜单..."
        main_menu
    elif [ "$choice" == "12" ]; then
         # 如果重装被取消，返回菜单
